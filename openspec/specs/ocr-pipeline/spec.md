...
Text recognition from manga images using the manga-ocr and PaddleOCR pipelines.

## Requirements

### Requirement: Recognize text in manga images
The system SHALL use the manga-ocr ONNX pipeline (or PaddleOCR high-accuracy path when enabled) for all supported source languages. The system SHALL support Japanese, English, and Traditional Chinese text recognition. For Japanese, the system SHALL handle both vertical (tategaki) and horizontal text in manga speech bubbles.
...
- **THEN** the system uses the same manga-ocr/PaddleOCR pipeline used for Japanese
