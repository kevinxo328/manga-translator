...
- 2 / 139 visible false positives, both at `conf < 0.6`, while all true positives sit at `conf ≥ 0.77`; raising the shared confidence threshold from `0.40` to `0.55` eliminates the failures without losing any true positives.
...
- Raise `ComicTextDetectorService.confidenceThreshold` from `0.40` to `0.55`.
...
- A title/SFX position heuristic. U13 shows the failure rate is 1.4 % and well-correlated with low confidence; a heuristic costs more than it saves until U15 broadens the corpus.
...
### D4. Bump confidence threshold to `0.55` as a single named constant

U13 audit: every confirmed false positive has `conf ∈ {0.42, 0.55}`; every true positive has `conf ≥ 0.77`. Picking `0.55` (mid-safety margin) catches both FPs and stays well below the lowest TP.

Implementation: change `ComicTextDetectorService.confidenceThreshold` from `0.4` to `0.55`. Keep it as a named `private static let` so the value remains discoverable for tuning in a follow-up change (U15 calibration).
...
`BenchmarkReporter` already emits per-image structured records. The two new counters slot in alongside existing fields. Naming: `lowConfidenceDetections` (count of detector outputs with `0.40 ≤ conf < 0.60`, i.e. boxes the new threshold would now reject) and `invertedBubbles` (count where `isInverted == true`). Both serve as regression signals for U15's broader corpus audit later.

## Risks / Trade-offs

[Risk] The 1.4 % FP rate from U13 is from 17 pages from 2 sources. A more diverse corpus could surface FPs at higher confidence. → Mitigation: keep `confidenceThreshold` as a named constant; U15 (broader audit) tracked separately. The conf bump is reversible in one line if regressions appear.
...
[Risk] U13's "raise threshold to 0.55" finding contradicts U6's "no threshold tuning needed" finding. → Mitigation: U6's claim was about *per-class* tuning; U13 shows the *shared* threshold has slack. The two findings are compatible — the ROADMAP §3 entry for U6 has been amended to reflect that.
...
5. Bump `confidenceThreshold` from `0.4` to `0.55`.
