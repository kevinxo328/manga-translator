...
struct PaddleOCRIntegrationSpikeTests {
    ...
    func nativeEngineSpike() throws {
        guard ProcessInfo.processInfo.environment["ENABLE_PADDLEOCR_SPIKE_TESTS"] == "1" else {
            return
        }
        ...
        print("PaddleOCR spike first_run_ms=\(firstMs) warm_run_ms=\(secondMs) first_len=\(first.text.count) warm_len=\(second.text.count)")
